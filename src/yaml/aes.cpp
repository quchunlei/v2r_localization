...
unsigned char AES::FFmul(unsigned char a, unsigned char b)
{
  ...
  {
    bw[i] = bw[i - 1] << 1;
    if (bw[i - 1] & (unsigned char)0x80)
    {
      bw[i] ^= (unsigned char)0x1b;
    }
  }
  for (i = 0; i < 4; i++)
  {
    if ((a >> i) & (unsigned char)0x01)
    {
      res ^= (unsigned char)bw[i];
    }
  }
  return res;
}
...
void AES::MixColumns(unsigned char state[][4])
{
  ...
  for (c = 0; c < 4; c++)
  {
    ...
    {
      state[r][c] =
          FFmul(0x02, t[r]) ^ FFmul(0x03, t[(r + 1) % 4]) ^ FFmul(0x01, t[(r + 2) % 4]) ^ FFmul(0x01, t[(r + 3) % 4]);
    }
  }
}
...
void AES::InvMixColumns(unsigned char state[][4])
{
  ...
  for (c = 0; c < 4; c++)
  {
    ...
    for (r = 0; r < 4; r++)
    {
      state[r][c] = (unsigned char)(FFmul(0x0e, t[r]) ^ FFmul(0x0b, t[(r + 1) % 4]) ^ FFmul(0x0d, t[(r + 2) % 4]) ^
                                    FFmul(0x09, t[(r + 3) % 4]));
    }
  }
}
