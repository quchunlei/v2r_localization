...
namespace msf
{
OdomObserver2::OdomObserver2(const std::string name, const YAML::Node& param,
...
OdomObserver2::~OdomObserver2()
{
  stop();
}

void OdomObserver2::config(const YAML::Node& param)
{
...
void OdomObserver2::stop(void)
{
  if (observer_thread_active_.load())
  {
    ...
    WARNING << " OdomObserver2 existed" << RESET << END;
  }

  odom_ready_ = false;
}

bool OdomObserver2::ready(void)
{
  if (odom_ready_.load())
    return true;
  else
...
void OdomObserver2::addOdomData(const OdomData& odom)
{
...
void OdomObserver2::correctObs(const MsfState& state, Observation& obs)
{
}

bool OdomObserver2::applyEkfCorrect(const Observation& obs, const MsfState& state, MsfState& new_state)
{
  WARNING << "------Odom Correction2------ " << REND;  // << obs.seq << ": " << std::fixed << obs.timestamp << REND;
   ...
   */
  Eigen::Matrix<float, MEAS_SIZE, 19> Hx = Eigen::Matrix<float, MEAS_SIZE, 19>::Zero();  // 1X19
  Hx.block<1, 3>(0, 3) = v.transpose() / v.norm();
  ...
  MatR R = MatR::Identity() * vel_noise_ * vel_noise_;
...
Observation OdomObserver2::getObservation(const OdomData& odom_data)
{
  Observation obs;
...
void OdomObserver2::observerThread(void)
{
  ...
  while (observer_thread_active_.load())
  {
    ...
    if (waiting_num > 10)
    {
      WARNING << name() << "OdomObserver2: more than 10 ODOM data waiting!" << RESET << END;
    }
