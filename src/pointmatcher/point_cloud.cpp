                                                               ...
                                                               const bool isDense)
{
  ...
  for (auto it(rosMsg.fields.begin()); it != rosMsg.fields.end(); ++it)
  {
    ...
    {
      featLabels.emplace_back(Label(name, count));
      isFeature.emplace_back(true);
      fieldTypes.emplace_back(PM_types::FEATURE);
    }
    else if (name == "rgb" || name == "rgba")
    {
      descLabels.emplace_back(Label("color", (name == "rgba") ? 4 : 3));
      isFeature.emplace_back(false);
    }
    else if ((it + 1) != rosMsg.fields.end() && it->name == "normal_x" && (it + 1)->name == "normal_y")
    {
      if ((it + 2) != rosMsg.fields.end() && (it + 2)->name == "normal_z")
      {
        descLabels.emplace_back(Label("normals", 3));
        it += 2;
        isFeature.emplace_back(false);
        isFeature.emplace_back(false);
        fieldTypes.emplace_back(PM_types::DESCRIPTOR);
        fieldTypes.emplace_back(PM_types::DESCRIPTOR);
      }
      else
      {
        descLabels.emplace_back(Label("normals", 2));
        it += 1;
        isFeature.emplace_back(false);
        fieldTypes.emplace_back(PM_types::DESCRIPTOR);
      }
             ...
             boost::algorithm::ends_with(((it + 1)->name), "_splitTime_low32"))
    {
      ...
      timeLabels.emplace_back(Label(beginning, 1));
      it += 1;
      isFeature.emplace_back(false);
      fieldTypes.emplace_back(PM_types::TIME);
      fieldTypes.emplace_back(PM_types::TIME);
    }
    // else if (name == "stamps")
    else if (name == "time")
    {
      timeLabels.emplace_back(Label(name, count));
      isFeature.emplace_back(false);
    }
    else
    {
      descLabels.emplace_back(Label(name, count));
      isFeature.emplace_back(false);
      fieldTypes.emplace_back(PM_types::DESCRIPTOR);
    }
  }
  ...
  for (auto it(rosMsg.fields.begin()); it != rosMsg.fields.end(); ++it, ++fieldId)
  {
    if (it->name == "rgb" || it->name == "rgba")
    {
      ...
      if (((it->datatype != PF::UINT32) && (it->datatype != PF::INT32) && (it->datatype != PF::FLOAT32)) ||
          (it->count != 1))
        throw runtime_error((boost::format("Colors in a point cloud must be a single element of size 32 bits, found "
                                           "%1% elements of type %2%") %
  ...
  if (isDense == false)
  {
    shared_ptr<typename PM::DataPointsFilter> filter(
        PM::get().DataPointsFilterRegistrar.create("RemoveNaNDataPointsFilter"));
...
typename PointMatcher<T>::DataPoints rosMsgToPointMatcherCloud(const sensor_msgs::LaserScan& rosMsg,
                                                               ...
                                                               const std::string& fixedFrame, const bool force3D,
                                                               ...
                                                               const bool addObservationDirection)
{
  ...
  {
    descLabels.emplace_back(Label("intensity", 1));
  ...
  {
    descLabels.emplace_back(Label("observationDirections", 3));
  }
  ...
  if (addTimestamps)
  {
    timeLabels.emplace_back(Label("time", 1));
    // timeLabels.emplace_back(Label("stamps", 1));
  }
  ...
  for (size_t i = 0; i < ranges.size(); ++i)
  {
    ...
    if (listener)
    {
      ...
      // wait for transform
      listener->waitForTransform(rosMsg.header.frame_id, fixedFrame, curTime, ros::Duration(0.1));
      ...
      try
      {
        listener->transformPoint(fixedFrame, rosMsg.header.stamp, pin, fixedFrame, p_out);

        if (addObservationDirection)
        {
          listener->transformPoint(fixedFrame, curTime, s_in, fixedFrame, s_out);
        }
      }
  ...
  if (addTimestamps)
  {
    ...
    for (size_t i = 0; i < ranges.size(); ++i)
    {
      ...
      const boost::posix_time::time_duration duration =
          boost::posix_time::seconds(curTime.sec) + boost::posix_time::nanoseconds(curTime.nsec);
...
template PointMatcher<float>::DataPoints rosMsgToPointMatcherCloud<float>(const sensor_msgs::LaserScan& rosMsg,
                                                                          const tf::TransformListener* listener,
                                                                          const std::string& fixedFrame,
                                                                          const bool force3D, const bool addTimestamps,
                                                                          const bool addObservationDirection);
template PointMatcher<double>::DataPoints rosMsgToPointMatcherCloud<double>(
    const sensor_msgs::LaserScan& rosMsg, const tf::TransformListener* listener, const std::string& fixedFrame,
    const bool force3D, const bool addTimestamps, const bool addObservationDirection);

template <typename T>
sensor_msgs::PointCloud2 pointMatcherCloudToRosMsg(const typename PointMatcher<T>::DataPoints& pmCloud,
                                                   const std::string& frame_id, const ros::Time& stamp)
{
  ...
  for (auto it(pmCloud.featureLabels.begin()); it != pmCloud.featureLabels.end(); ++it)
  {
    ...
    rosCloud.fields.emplace_back(pointField);
  ...
  for (unsigned pt(0); pt < rosCloud.width; ++pt)
  {
    ...
    if (isDescriptor)
    {
      if (hasColor)
      {
        ...
        fPtr += 4;
        // after color
        memcpy(fPtr, reinterpret_cast<const uint8_t*>(&pmCloud.descriptors(postColorPos, pt)),
